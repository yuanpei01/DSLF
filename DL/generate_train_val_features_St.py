def gen_tr(repeattimes, data, method="cos", threshold=0.9, seed=None):
    ...
    from process.countdataset import print_dataset_statistics
    ...
    # 在 read_dataset 之后，cleandata 之前添加
    original_total_users = len(set(X_train_tid + X_dev_tid + X_test_tid))  # 粗略估计
    original_total_comments = len(X_train_content) + len(X_dev_content) + len(X_test_content)
    ...
    # 在 cleandata 之后
    cleaned_total_users = len(set(X_train_tid + X_dev_tid + X_test_tid))
    cleaned_total_comments = len(X_train_content) + len(X_dev_content) + len(X_test_content)

    print(f"\n原始用户数: {original_total_users}, 清洗后: {cleaned_total_users}")
    print(f"原始评论数: {original_total_comments}, 清洗后: {cleaned_total_comments}")
    
    stats = print_dataset_statistics(data, y_train, y_dev, y_test)
    write_countdataset(data, y_train, y_dev, y_test, stats,cleaned_total_users,cleaned_total_comments)
    ...
    # 先查看数据结构，确定如何提取用户ID
    print("\n=== 数据结构检查 ===")
    print(f"X_train_tid[0] 类型: {type(X_train_tid[0])}, 值: {X_train_tid[0]}")
    print(f"X_train_replies[0] 类型: {type(X_train_replies[0])}")
    if len(X_train_replies[0]) > 0:
        print(f"第一条回复类型: {type(X_train_replies[0][0])}")
        print(f"第一条回复内容: {X_train_replies[0][0]}")
    ...
    for epoch in range(num_epochs1):
        ...
        with open(path_glan_log(_method, data, repeattimes, threshold), "a+") as f:
            f.write(
