...
#    # Ensures that operations are deterministic on GPU
...
# Your GLAN class and other code...
import torch
...
class GLAN(NeuralNetwork):
    ...
    def replies_encoder(self, X_replies, X_source):
        bsz, src_len, num_words, dim = X_replies.size()

        # 对 source 特征进行均值池化
        source_mean = X_source.mean(dim=1)  # (批次数, 特征维度)
        replies_mean = X_replies.mean(dim=2)  # (批次数, source长度, 词嵌入维度)

        # 计算余弦相似度
        cos_sim = F.cosine_similarity(source_mean.unsqueeze(1), replies_mean, dim=2)  # (批次数, source长度)

        # 对余弦相似度进行 softmax 处理，将其作为加权系数
        weights = F.softmax(cos_sim, dim=1)  # (批次数, source长度)

        # 为每条 reply 的特征加权
        X_replies_weighted = X_replies * weights.unsqueeze(2).unsqueeze(3)  # (批次数, source长度, 词长度, 词嵌入维度)

        # 变换维度以适应卷积层
        X_replies_weighted = X_replies_weighted.view(bsz * src_len, num_words, dim)
        X_replies_weighted = X_replies_weighted.permute(0, 2, 1)  # (批次数 * source长度, 词嵌入维度, 词长度)

        # 提取 replies 特征
        conv_block = []
        for _, (Conv, max_pooling) in enumerate(zip(self.convs_replies, self.max_poolings)):
            act = self.relu(Conv(X_replies_weighted))
            pool = max_pooling(act)
            pool = torch.squeeze(pool)
            conv_block.append(pool)

        conv_feature = torch.cat(conv_block, dim=1)  # (批次数 * source长度, 特征维度)
        features = self.dropout(conv_feature)
        features = features.view(bsz, src_len, features.size(-1))  # (批次数, source长度, 特征维度)
    ...
    def local_attention_network(self, X_source, X_replies, mask):
        ...
        X_replies_feat = self.replies_encoder(X_replies, X_source)
    ...
    def global_graph_encoding(self, X_tid):
        ...
        # Check shapes before applying dropout
        #print("Before Dropout - Edge Index Shape:", edge_index.shape)
        #if edge_weight is not None:
        #    print("Before Dropout - Edge Weight Shape:", edge_weight.shape)
        #    assert edge_weight.size(0) == edge_index.size(1), \
        #        "Edge weight size does not match edge index size."
